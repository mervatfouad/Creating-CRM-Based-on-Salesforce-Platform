...
class DataTableHelper
{
    ...
    public static function translations(){
        try{
            ...
            return \File::get(public_path('bap/js/trans/datatable/'.app()->getLocale().'.json'));
    ...
    public static function renderLink($column, $record, $columnProperties, $route)
    {
        ...
        $displayColumn = str_split($displayColumn,30);

        if(strlen($oldDisplayColumn) >= 30){
            $displayColumn = $displayColumn[0].'...';
        }else{
            $displayColumn = $displayColumn[0];
        }
